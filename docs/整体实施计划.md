# 梨苑儿（BITerStore）校内二手书交易平台整体实施计划
...
梨苑儿第一阶段应被定义为：**仅面向北京理工大学已认证成员、以信息撮合和校内当面交易为核心、不承接线上支付的二手书平台**。
...
4. 校园身份认证必须通过 BIT101/学校授权的认证服务完成；梨苑儿不保存学校统一身份认证密码，也不自行模拟或抓取学校登录流程。
...
`BIT101-dev/BIT-Login` 当前主分支则已经提供较完整的 Kotlin/Ktor 认证服务：异步 challenge、短信二次验证、短期 session，以及面向指定 `audience` 签发的 Ed25519 注册 JWT。它可以作为梨苑儿的校园认证适配器，优先使用 `/api/auth/*` 与 `/registration-token`，无需复制旧版 WebVPN 实现。
...
- 服务同时暴露成绩、课程、Cookie 等接口，梨苑儿专用实例应通过代码或网关只开放 `/api/auth/*` 和健康检查；
- 确认部署责任、上游认证变化、故障联络、限流以及用户凭据处理披露。

BIT101 前后端采用 AGPL-3.0，当前梨苑儿前端采用 Apache-2.0。可以参考其公开设计和文档；若复制、修改或部署其代码，必须先做许可证兼容性评估，不能默认保持梨苑儿原有许可不变。
...
数据库中将两者绑定到一个梨苑儿用户，而不是把学号直接当作主键。

### 5.2 确定方案：BIT-Login 注册 JWT

在取得使用许可后，部署梨苑儿专用的 BIT-Login 实例，采用以下接口：
...
5. `DELETE /api/auth/{challenge_id}`：梨苑儿完成注册/绑定后立即销毁 challenge 和关联会话。

梨苑儿后端仅信任 registration token，不接收学校密码、短信码或教务 Cookie。它必须验证 JWT 的 Ed25519 签名、`iss`、`aud=biterstore`、`exp`、`iat`、`purpose=registration`、`sub` 和 `jti`，并将 `jti` 作为一次性凭证消费，防止重放。
...
- BIT-Login 如何处理和暂存密码、短信码、学校 Cookie/session，梨苑儿需要向用户披露哪些事项？
...
- [梨苑儿当前前端（BITerStore）](https://github.com/AnonymousUser443/BITerStore)：现有移动端产品原型和视觉基线。
