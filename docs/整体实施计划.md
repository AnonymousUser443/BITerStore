...
> 当前决策（2026-08-26）：停止 Taro 页面迁移并移除实验代码，以现有 `web/` H5 作为唯一客户端继续开发。本文保留 Taro 方案内容作为技术选型记录；若未来仍需要微信小程序入口，优先评估 HTTPS H5 + 合规 `web-view` 壳及微信业务域名要求。
...
├─ clients/
│  └─ miniapp/          # Taro React，输出 weapp 与 h5
├─ packages/
