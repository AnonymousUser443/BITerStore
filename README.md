...
当前锁定 Taro 4.2.1 的传递依赖仍有 npm audit 上游告警，且完整复用 Golden Reference 样式后的 H5 入口约 431 KiB。它们不阻塞离线演示，但属于正式生产化前必须解决的升级与性能门禁；不要对 lockfile 执行不兼容的 `npm audit fix --force`。
