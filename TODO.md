...
- [x] 按最新产品决策统一双端底栏为“首页 / 分类 / 发布 / 消息 / 我的”，分类页内部继续提供搜索与筛选能力
...
- [x] 融合远端 PR #6 的校园 CAS、真实数据接口、服务端与管理后台基线，并保留 H5/微信自动化守门
- [x] 按最新产品决策将 H5 与微信底栏统一为“首页 / 分类 / 发布 / 消息 / 我的”
- [x] 使用已登录 Edge 对线上 H5 进行路由、网络、DOM、控制台与截图调试，定位顶栏空操作、欢迎页地址不同步和桌面个人中心入口裁切
- [x] 修复 H5 顶栏通知/头像/更多菜单、欢迎页规范地址、分类页相机反馈、首页真实空态、发布页本地 Tobby 填表和桌面个人中心入口裁切
- [x] 为 H5 视觉门禁加入确定性登录/API fixture、顶栏跳转断言和个人中心裁切检查，最终 17/17 路由/视口无诊断
- [x] 去除 H5 重复纸张背景产物并无感重压纹理资源；首屏 352.3 KiB、最大资源 205.7 KiB、总资源 3067.7 KiB 均通过固定性能门槛
- [x] 完成融合后的微信开发者工具 6/6 核心流程回归；应用 console error、exception 均为 0
- [x] 修复服务端安装后 Prisma Client 生成与静态检查命令；服务端 18/18 测试、类型检查和 Nest 构建通过，管理后台 TypeScript/Vite 构建通过
- [x] 将 H5 画布从固定手机宽度升级为 320–1920px 流式布局，覆盖窄屏手机、大屏手机、平板、桌面与超宽桌面
- [x] 为 480–699px 补齐连续扩展画布，为 700px+ 建立自适应内容网格，为 1024px+ 建立侧栏导航与宽屏工作区
- [x] 修复 844×390 低高度横屏底栏遮挡，横屏平板自动切换紧凑侧栏；修复桌面单商品卡片被拉伸和详情列溢出
- [x] 将 H5 视觉门禁扩展为 51 个路由/视口组合，覆盖 320、360、390、430、480、600、768、844×390、1024、1280、1440、1600 与 1920px
- [x] 新增画布边界、内容横向溢出、导航方向、网格重排与宽屏个人中心列数断言，51/51 无诊断通过
- [x] 保持固定性能门槛并优化纸张纹理；首屏 358.8 KiB、最大资源 205.7 KiB、总资源 3071.4 KiB 全部通过
- [x] 多尺寸改造后完成 miniProgram 18 项单测、H5/WeApp 构建、微信 6/6 E2E，以及 web Golden Reference 17 项测试与生产构建
...
- [ ] Phase 7：已完成 320–1920px 共 51 项 H5 路由/视口回归及微信模拟器 6/6 功能回归；待至少一台实体微信真机冒烟
...
- [x] 多尺寸布局后的 H5 生产入口为 358.8 KiB，最大资源 205.7 KiB、总资源 3071.4 KiB，已通过 400 KiB 首屏、384 KiB 单资源及 3 MiB 总静态资源门槛；正式上线前仍可继续拆分非首屏组件。
- [ ] 当前本机为 Node 25.2.1，而仓库锁定并要求 Node 22.13.x；本轮构建和自动化均已通过，但团队日常开发和最终真机验收应切换到锁定版本，避免 `miniprogram-automator` 的非受支持运行时风险。
- [ ] 管理后台当前只有类型/生产构建门禁，尚未建立组件或交互自动化测试；后端管理接口稳定后补齐登录、列表加载和处置确认用例。
...
- 当前已接入搭档维护的校园认证与真实业务 API；前端修复不得绕过 Repository/平台适配边界，支付与真实 AI 仍不在范围内。
