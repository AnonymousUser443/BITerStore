...
const authorized = (isbn) => new Request(`https://worker.test/isbn/${isbn}`, { headers: { Authorization: `Bearer ${proxyToken}` } })
const configured = (value = {}) => ({ PROXY_TOKEN: proxyToken, ...value })
...
test('fails closed when the shared token is not configured', async () => {
...
test('rejects invalid ISBN checksums before contacting an upstream', async () => {
  ...
  const response = await worker.fetch(authorized('9787115428029'), configured(), { waitUntil() {} })
...
test('accepts ISBN-10 and matches an upstream ISBN-13 edition', async () => {
  ...
  const response = await worker.fetch(
    authorized('7115428026'),
    configured({ ISBN_WORK_APP_KEY: 'isbn-work-secret' }),
    { waitUntil() {} },
  )
...
test('selects the exact Google Books ISBN instead of the first fuzzy result', async () => {
  ...
  const response = await worker.fetch(
    authorized('9787115428028'),
    configured({ GOOGLE_BOOKS_API_KEY: 'google-secret' }),
    { waitUntil() {} },
  )
  assert.equal(response.status, 200)
...
test('selects the exact Open Library search ISBN instead of the first fuzzy result', async () => {
  ...
  const response = await worker.fetch(authorized('9787115428028'), configured(), { waitUntil() {} })
  assert.equal(response.status, 200)
  assert.equal((await response.json()).title, '精确书名')
  assert.equal(call, 2)
})
