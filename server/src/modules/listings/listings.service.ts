...
export class ListingsService {
  ...
  favorites(userId: string) { return this.prisma.favorite.findMany({ where: { userId }, orderBy: { createdAt: 'desc' }, include: { listing: { include: { images: { where: { uploadedAt: { not: null } }, orderBy: { sortOrder: 'asc' } }, seller: { select: { id: true, nickname: true, avatarUrl: true, campus: true, campusStatus: true, bio: true } } } } } }).then((rows) => rows.map((row) => this.present(row.listing))) }
}
