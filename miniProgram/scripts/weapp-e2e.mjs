...
async function pageAt(pathname, timeout = 6000) {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      lastRoute = page?.path || ''
      if (lastRoute === pathname) return page
...
try {
  ...
  const results = []
  results.push(await scenario('onboarding-home', async () => { await app.reLaunch('/pages/welcome/index'); let page = await pageAt('pages/welcome/index'); await sleep(250); page = await tapForRoute(await required(page, '#e2e-welcome-start'), 'pages/onboarding/index'); await shot('visual-onboarding'); for (let i = 0; i < 3; i += 1) { const next = await required(page, '#e2e-onboarding-next'); if (i < 2) { await next.tap(); await sleep(180) } else page = await tapForRoute(next, 'pages/home/index') } await required(page, '#e2e-home-search-entry'); await shot('visual-home'); if (!await stored('onboarding')) throw new Error('引导完成状态未持久化') }))
