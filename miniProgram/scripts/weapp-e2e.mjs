...
try {
  ...
  results.push(await scenario('bit-login-private-route-blocked', async () => { const response = await probeRequest('https://www.biterstore.top/bit-login/openapi.json'); if (!response?.ok || response.statusCode !== 404) throw new Error(`BIT-Login 私有路由边界异常（${response?.statusCode || response?.error || 'unknown'}）`) }))
  results.push(await scenario('production-api-request-domain', async () => { const response = await probeRequest('https://www.biterstore.top/api/v1/health'); if (!response?.ok || response.statusCode !== 200) throw new Error(`生产 API 请求失败（${response?.statusCode || response?.error || 'unknown'}）`) }))
