...
try {
  ...
  results.push(await scenario('bit-login-private-route-blocked', async () => { const response = await probeRequest('https://store.young581.com/bit-login/openapi.json'); if (!response?.ok || response.statusCode !== 404) throw new Error(`BIT-Login 私有路由边界异常（${response?.statusCode || response?.error || 'unknown'}）`) }))
