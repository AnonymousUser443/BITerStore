...
export async function checkBackNavigation(client, preview, artifactDir) {
  const trace = (label) => { if (process.env.BITERSTORE_H5_TRACE === '1') console.error(`[h5-back] ${label}`) }
  ...
  await Promise.race([loaded, pause(5000)])
  trace('home-loaded')
  await pause(1000)
