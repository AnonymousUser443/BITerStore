...
export default function HomePage() {
  ...
  return <AppShell active='home' className='home-page'>
    ...
    <View className='hero-card'><View className='hero-copy'><Text className='eyebrow'>书页轻翻 · 好物续航</Text><Text className='hero-title'>以书会友{'\n'}共享知识之美</Text><Text className='hero-description'>让每一本闲置书，遇见下一位需要它的人。</Text><Button className='hero-button' onClick={() => openSearch()}>探索好书 <Text>→</Text></Button></View><Image className='hero-tobby' src={bundledAsset('tobby-hello')} mode='aspectFit' /></View>
    <View className='section-block'><View className='section-title'><Text className='section-heading-text'>精选推荐</Text><Button className='section-more' onClick={() => openSearch()}>查看全部 <Glyph name='chevron' /></Button></View>{items === undefined ? <View className='inline-loading'>托比正在整理书架…</View> : items.length ? <View className='book-row'>{items.slice(0, 5).map((item) => <BookTile key={item.id} listing={item} href={`/pages/listing/detail?id=${item.id}`} />)}</View> : <View className='inline-state compact'><Image src={bundledAsset('tobby-question')} mode='aspectFit' /><Text className='inline-title'>还没有上架的书</Text><Button className='secondary-button' onClick={() => navigationAdapter.switchTab('/pages/search/index')}>看看其他分类</Button></View>}</View>
