...
export default function MyListingsPage() {
  ...
  return <AppShell title='我的发布' back className='simple-list-page'><View className='status-tabs'>{([['all', '全部'], ['available', '在售'], ['sold', '已售'], ['offline', '下架']] as const).map(([value, label]) => <Button className={tab === value ? 'active' : ''} onClick={() => setTab(value)} key={value}>{label}</Button>)}</View>{visible === undefined ? <View className='inline-state'><Image src={bundledAsset('tobby-search')} mode='aspectFit' /><Text className='inline-title'>托比正在整理书架…</Text></View> : visible.length ? visible.map((item) => <View className='manage-listing' key={item.id}><ListingCard listing={item} href={`/pages/listing/detail?id=${item.id}`} ownerView /><View className={`manage-listing-actions${['available', 'offline'].includes(item.status) ? '' : ' single-action'}`}>{['available', 'offline'].includes(item.status) && <Button className='secondary-button' onClick={() => change(item)}>{item.status === 'available' ? '标记已售' : '重新上架'}</Button>}<Button className='danger-button' onClick={() => remove(item)}>删除</Button></View></View>) : <View id='e2e-my-listings-empty' className='inline-state'><Image src={bundledAsset('tobby-question')} mode='aspectFit' /><Text className='inline-title'>这次没有找到合适的书</Text><Text className='inline-copy'>换个状态，或者发布一本闲置书吧。</Text></View>}<Button className='floating-add' onClick={() => navigationAdapter.switchTab('/pages/publish/index')}>＋ 发布一本书</Button></AppShell>
}
