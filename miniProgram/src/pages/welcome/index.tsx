...
export default function WelcomePage() {
  ...
  return <View className='phone-shell welcome-page'>
    ...
    <View className='welcome-copy'><Text className='welcome-greeting'>你好呀，我是托比 <Glyph name='leaf' /></Text><Text className='welcome-title'>欢迎来到你的{'\n'}<Text className='welcome-accent'>校园二手书小站</Text></Text><Text className='welcome-description'>搜索闲置教材、发布旧书、站内联系，{'\n'}在校内安心完成交易。</Text></View>
    <Image className='welcome-tobby' src={bundledAsset('tobby-master-transparent')} mode='aspectFit' />
    ...
    <View className='welcome-actions'><Button id='e2e-welcome-start' className='primary-button' onClick={start}>进入梨苑儿</Button><Button className='secondary-button' onClick={() => navigationAdapter.go('/pages/onboarding/index')}>先看看如何使用</Button><Text className='welcome-footnote'>❧ 北理工校内试运行中 ❧</Text></View>
    <Modal open={notice} title='欢迎来到新版梨苑儿' onClose={() => { void demoRepository.acknowledgeResetNotice(); setNotice(false) }}><Text>新版使用独立演示数据，不会读取或删除旧 H5 的收藏、草稿和消息。</Text></Modal>
  </View>
}
