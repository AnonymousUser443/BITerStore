...
export default function ProfilePage() {
  ...
  const openEditor = () => { if (!user) return; setDraft({ name: user.name, campus: user.campus, bio: user.bio, avatar: user.avatar }); setEditing(true) }
  ...
  const saveProfile = async () => {
    ...
    if (draft.bio.length > 160) return feedbackAdapter.toast('个人简介不能超过 160 个字符')
    setSaving(true)
    try { setUser(await demoRepository.updateProfile({ ...draft, name })); setEditing(false); await feedbackAdapter.toast('个人资料已保存') }
  ...
  return <AppShell title='我的' active='profile' className='profile-page'>
    ...
    {editing && draft && <View className='profile-editor-mask' onClick={() => !saving && setEditing(false)}><View className='profile-editor' onClick={(event) => event.stopPropagation()}><View className='editor-handle' /><View className='editor-title'><Text>编辑个人资料</Text><Button onClick={() => !saving && setEditing(false)}>×</Button></View><View className='profile-avatar-editor'><Avatar user={{ ...user, avatar: draft.avatar }} size={82} /><View><Button className='secondary-button' onClick={chooseAvatar}><Glyph name='camera' /> 更换头像</Button>{draft.avatar && <Button className='avatar-clear' onClick={() => setDraft({ ...draft, avatar: undefined })}>移除头像</Button>}</View></View><View className='profile-edit-field'><Text>昵称</Text><Input id='e2e-profile-name' maxlength={24} value={draft.name} onInput={(event) => setDraft({ ...draft, name: event.detail.value })} /></View><View className='profile-edit-field'><Text>校区</Text><Picker mode='selector' range={campuses} value={Math.max(0, campuses.indexOf(draft.campus))} onChange={(event) => setDraft({ ...draft, campus: campuses[Number(event.detail.value)] })}><View className='select-control'>{draft.campus}校区</View></Picker></View><View className='profile-edit-field'><Text>个人简介</Text><Textarea maxlength={160} value={draft.bio} onInput={(event) => setDraft({ ...draft, bio: event.detail.value })} placeholder='介绍一下自己吧' /></View><Button id='e2e-profile-save' className='primary-button profile-save' disabled={saving} onClick={saveProfile}>{saving ? '保存中…' : '保存个人资料'}</Button></View></View>}
  </AppShell>
}
