...
export default function SearchPage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      void demoRepository.listListingsPage(filters).then((page) => {
        if (!active) return
        setItems(page.items)
        setNextCursor(page.nextCursor)
      }).catch(() => {
        if (active) void Taro.showToast({ title: '加载失败，请稍后重试', icon: 'none' })
      })
  ...
  const loadMore = async () => {
    ...
    setLoadingMore(true)
    try {
      const page = await demoRepository.listListingsPage(filters, nextCursor)
      setItems((current) => [...current, ...page.items.filter((item) => !current.some((existing) => existing.id === item.id))])
      setNextCursor(page.nextCursor)
    } catch {
      void Taro.showToast({ title: '加载更多失败，请稍后重试', icon: 'none' })
    } finally {
  ...
  return <AppShell active='search' className='search-page'>
    ...
    <View className='results-heading'><Text>已加载 <Text>{items.length}</Text> 本书</Text><Text>{filters.availableOnly ? '只显示可交易' : filters.sort}</Text></View>
    {items.length ? <><View className='listing-stack'>{items.map((item) => <ListingCard key={item.id} listing={item} href={`/pages/listing/detail?id=${item.id}`} favorite={favorites.includes(item.id)} onFavorite={() => toggleFavorite(item.id)} onContact={() => contact(item)} />)}</View>{nextCursor && <Button id='e2e-catalog-load-more' className='secondary-button catalog-load-more' disabled={loadingMore} onClick={loadMore}>{loadingMore ? '正在加载…' : '加载更多'}</Button>}</> : <View className='inline-state'><Image src={bundledAsset('tobby-question')} mode='aspectFit' /><Text className='inline-title'>这次没有找到合适的书</Text><Text className='inline-copy'>换个关键词，或者发布一条求书心愿吧。</Text><Button className='secondary-button' onClick={() => navigationAdapter.go('/pages/states/index?type=no-results')}>查看空状态</Button></View>}
