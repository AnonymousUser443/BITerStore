...
import { AppShell, FilterDrawer, ListingCard, Tobby } from '@/components/ui'
...
export default function SearchPage() {
  ...
  return <AppShell active='search' className='search-page'>
    <View className='search-box category-search'><Glyph name='search' /><Input id='e2e-search-input' value={filters.query} onInput={(e) => setFilters({ ...filters, query: e.detail.value })} placeholder='搜索书名 / 作者 / ISBN / 课程' /><Glyph name='camera' /></View>
    ...
    <View className='search-hint'><Tobby mood='search' /><View><Text>托比提示</Text><Text>组合筛选，找书更快更准。</Text></View></View>
    ...
    {items.length ? <View className='listing-stack'>{items.concat(items).map((item, index) => <ListingCard key={`${item.id}-${index}`} listing={item} onTap={() => navigationAdapter.go(`/pages/listing/detail?id=${item.id}`)} />)}</View> : <View className='empty'>没有找到匹配的书，换个关键词试试。</View>}
