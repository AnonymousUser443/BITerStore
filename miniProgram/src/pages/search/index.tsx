...
export default function SearchPage() {
  ...
  return <AppShell active='search' className='search-page'>
    ...
    <View className='category-chips'>{categories.map((category, index) => <Button key={category} className={`${index === 0 ? 'chip active' : 'chip'} ${filters.category === category ? 'selected' : ''}`} onClick={() => setFilters({ ...filters, category })}>{category}</Button>)}</View>
    ...
    {items.length ? <View className='listing-stack'>{items.map((item) => <ListingCard key={item.id} listing={item} href={`/pages/listing/detail?id=${item.id}`} favorite={favorites.includes(item.id)} onFavorite={() => toggleFavorite(item.id)} onContact={() => contact(item)} />)}</View> : <View className='inline-state'><Image src='/assets/tobby-question.webp' mode='aspectFit' /><Text className='inline-title'>这次没有找到合适的书</Text><Text className='inline-copy'>换个关键词，或者发布一条求书心愿吧。</Text><Button className='secondary-button' onClick={() => navigationAdapter.go('/pages/states/index?type=no-results')}>查看空状态</Button></View>}
