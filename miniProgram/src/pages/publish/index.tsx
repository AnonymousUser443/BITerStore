...
export default function PublishPage() {
  ...
  const generate = async () => {
    if (__API_URL__ && !__BITERSTORE_E2E__) { await feedbackAdapter.toast('辅助填写服务暂未开放'); return setStep(2) }
    setAiLoading(true)
    const result = await listingAssistant.generate(draft)
    patch({ ...result, author: draft.author || '同济大学数学系 编', isbn: draft.isbn || '978-7-5608-9493-7', category: '教材教辅', course: draft.course || '高等数学', price: draft.price || '26', originalPrice: draft.originalPrice || '49.8', condition: '九成新' })
    setAiLoading(false)
    setStep(2)
  }
  ...
  const removeImage = async (id: string) => { await mediaAdapter.remove([id]); patch({ mediaIds: draft.mediaIds.filter((value) => value !== id) }) }
