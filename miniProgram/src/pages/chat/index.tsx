...
export default function ChatPage() {
  ...
  return <AppShell title={user?.name || '消息'} back noNav className='chat-page'>
    ...
    <View className='chat-safety'>◈ 站内沟通更安全 · 当面交易请确认书况</View>
    ...
    <View className='chat-composer'><Button id='e2e-message-image' onClick={sendImage}>▧</Button><Button>♡</Button><Input id='e2e-message-input' value={text} onInput={(event) => setText(event.detail.value)} onConfirm={send} placeholder='输入消息…' /><Button id='e2e-message-send' className='send-button' onClick={send}>发送</Button></View>
  </AppShell>
}
