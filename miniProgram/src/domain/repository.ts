...
export interface DemoRepository {
  ...
  updateListingStatus(id: string, status: ListingStatus): Promise<ListingStatus>; deleteListing(id: string): Promise<void>; listMyListings(): Promise<Listing[]>; listMyListingsPage(cursor?: string): Promise<{ items: Listing[]; nextCursor: string | null }>; countMyListings(): Promise<number>; peekMyListings(): Listing[] | undefined;
...
const localRepository: DemoRepository = {
  ...
  async updateListingStatus(id, status) { const items = await storageAdapter.get(KEYS.listings, seedListings); listingCache = items.map((x) => x.id === id ? { ...x, status } : x); await storageAdapter.set(KEYS.listings, listingCache); return status },
