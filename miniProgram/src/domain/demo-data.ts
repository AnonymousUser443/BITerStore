import type { Book, ChatThread, Notification, NotificationType, User } from './types'
...
export const users: User[] = [
  ...
  { id: 'user-lin', name: '林小暖', campus: '中关村', verified: true, bio: '数学与生活都要慢慢理解。', responseTime: '30 分钟内回复', avatarTone: 'peach', avatar: 'avatarLin' },
  { id: 'user-zhou', name: 'Oliver_周', campus: '良乡', verified: true, bio: '计院同学，教材循环利用中。', responseTime: '1 小时内回复', avatarTone: 'blue', avatar: 'avatarZhou' },
  { id: 'user-jian', name: '简一一', campus: '珠海', verified: true, bio: '喜欢文学，也喜欢把书分享出去。', responseTime: '当天回复', avatarTone: 'gold', avatar: 'avatarJian' },
...
export const notificationDetails: Record<NotificationType, Array<{ source: string; text: string; time: string; target: string }>> = {
  like: [
    { source: '林小暖', text: '赞了你发布的《高等数学（第七版）上册》', time: '10:18', target: 'book:math-7' },
    { source: '简一一', text: '赞了你的校园书单「期末复习好书」', time: '昨天 18:42', target: 'favorites' },
    { source: 'Oliver_周', text: '赞了你分享的旧书循环动态', time: '周五 20:06', target: 'profile' },
  ],
  comment: [
    { source: 'Oliver_周', text: '评论：书的笔记多吗？方便拍一下目录页吗？', time: '10:05', target: 'book:data-c' },
    { source: '林小暖', text: '评论：这本高数正好是我需要的版本～', time: '昨天 21:30', target: 'book:math-7' },
    { source: 'Leo 学长', text: '评论：中关村校区也可以约时间自取。', time: '昨天 16:12', target: 'chat:thread-leo' },
  ],
  system: [
    { source: '校园交易安全提醒', text: '请尽量选择校内公共区域当面交易，确认书况后再付款。', time: '今天 09:00', target: 'state:maintenance' },
    { source: 'BITerStore 试运行公告', text: '演示数据与状态体验入口已经更新完成。', time: '昨天 12:00', target: 'profile' },
  ],
  follow: [{ source: '简一一', text: '关注了你，之后发布的新书会更容易被她发现。', time: '昨天 15:26', target: 'chat:thread-jian' }],
}

export const getUser = (id: string) => users.find((user) => user.id === id) ?? users[0]
