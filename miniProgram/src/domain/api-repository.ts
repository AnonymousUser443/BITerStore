...
const statusFromApi = (value: string): ListingStatus => ({ ACTIVE: 'available', RESERVED: 'available', SOLD: 'sold', OFF_SHELF: 'offline', BLOCKED: 'offline', DRAFT: 'draft', PENDING_REVIEW: 'reviewing', CHANGES_REQUESTED: 'changes_requested' }[value] || 'offline') as ListingStatus
const statusToApi = (value: ListingStatus) => ({ available: 'ACTIVE', sold: 'SOLD', offline: 'OFF_SHELF', draft: 'DRAFT', reviewing: 'PENDING_REVIEW', changes_requested: 'CHANGES_REQUESTED' }[value])
...
function normalizeMediaUrl(value: unknown) {
  ...
  const legacyPrefix = 'https://store.young581.com/api/v1'
  return url.startsWith(legacyPrefix) ? `${__API_URL__}${url.slice(legacyPrefix.length)}` : url
}
function listing(value: any): Listing { const version = Number(value?.version); return { id: String(value?.id || ''), title: String(value?.title || '未命名书籍'), author: String(value?.author || ''), isbn: String(value?.isbn || ''), category: String(value?.category || '其他'), course: String(value?.course || ''), price: Number(value?.priceCents || 0) / 100, originalPrice: Number(value?.originalPriceCents ?? value?.priceCents ?? 0) / 100, condition: value?.condition || '八成新', campus: campuses.has(value?.campus) ? value.campus : '未设置', description: String(value?.description || ''), status: statusFromApi(value?.status), ...(value?.moderationReason ? { moderationReason: String(value.moderationReason) } : {}), sellerId: String(value?.sellerId || ''), seller: value?.seller ? profile(value.seller) : undefined, createdAt: String(value?.createdAt || ''), ...(Number.isFinite(version) ? { version } : {}), tags: Array.isArray(value?.tags) ? value.tags : [], tone: 'sage', mediaIds: (value?.images || []).map((image: any) => String(image.id)).filter(Boolean), imageUrls: (value?.images || []).map((image: any) => normalizeMediaUrl(image.url)).filter(Boolean) } }
...
export const apiRepository: DemoRepository = {
  ...
  async getListing(id, options) { const encodedId = encodeURIComponent(id); try { return remember(listing(await apiRequest(options?.owner ? `/listings/mine/${encodedId}` : `/listings/${encodedId}`))) } catch (cause) { const cached = listingCache.get(listingCacheKey(id)); const currentUserId = sessionStore.peek()?.user.id; if (cached && (!options?.owner || cached.sellerId === currentUserId)) return cached; throw cause } }, peekListing(id) { return listingCache.get(listingCacheKey(id)) },
  async toggleFavorite(id) { const key = listingCacheKey(id); const enabled = !favoriteIds.has(key); await apiRequest(`/listings/${encodeURIComponent(id)}/favorite`, { method: enabled ? 'PUT' : 'DELETE' }); if (enabled) favoriteIds.add(key); else favoriteIds.delete(key); const current = readSnapshot<Listing[]>('favorites') || []; const cached = listingCache.get(key); await writeSnapshot('favorites', enabled && cached ? [cached, ...current.filter((item) => item.id !== id)] : current.filter((item) => item.id !== id)); return enabled },
  ...
  async updateListingStatus(id, status) {
    ...
    const current = await apiRequest<{ version?: number }>(`/listings/mine/${encodedId}`)
    if (!Number.isFinite(Number(current.version))) throw new Error('商品版本信息缺失，请刷新后重试')
    const updated = await apiRequest<{ status: string; version: number }>(`/listings/${encodedId}/status`, { method: 'POST', data: { status: statusToApi(status), version: Number(current.version) } })
    const actual = statusFromApi(updated.status)
    const apply = (item: Listing) => item.id === id ? { ...item, status: actual, version: updated.version } : item
    const cached = listingCache.get(listingCacheKey(id))
    if (cached) remember(apply(cached))
    for (const kind of ['my-listings', 'favorites'] as const) {
      const items = readSnapshot<Listing[]>(kind)
      if (items) await writeSnapshot(kind, items.map(apply))
    }
    return actual
  },
  ...
  async getThread(id) { const current = await sessionStore.get(); const encodedId = encodeURIComponent(id); const [messages, found] = await Promise.all([apiRequest<{ items: any[]; olderCursor?: string | null; blocked?: boolean }>(`/conversations/${encodedId}/messages` + apiQuery({ limit: 30 })), apiRequest<any>(`/conversations/${encodedId}`)]); if (!found || found.id !== id) throw new Error('会话不存在'); const fresh = thread({ ...found, messages: messages.items, olderCursor: messages.olderCursor, blocked: messages.blocked ?? found.blocked }, current?.user.id); const cached = readSnapshot<ChatThread>('thread', id); const value = cached ? { ...fresh, olderCursor: cached.olderCursor ?? fresh.olderCursor, messages: mergeMessages(cached.messages, fresh.messages) } : fresh; const latest = messages.items?.at(-1); if (latest?.id) await apiRequest(`/conversations/${encodedId}/read`, { method: 'POST', data: { messageId: String(latest.id) } }).catch(() => undefined); value.unread = 0; await persistThread(value); return value },
  async loadOlderMessages(threadId, before) { const page = await apiRequest<{ items: any[]; olderCursor?: string | null }>(`/conversations/${encodeURIComponent(threadId)}/messages` + apiQuery({ before, limit: 30 })); const items = sortMessagesChronologically(page.items.map(message)); const cached = readSnapshot<ChatThread>('thread', threadId); if (cached) await persistThread({ ...cached, olderCursor: page.olderCursor || null, messages: mergeMessages(cached.messages, items) }); return { items, olderCursor: page.olderCursor || null } },
  peekThread(id) { return readSnapshot<ChatThread>('thread', id) },
  async sendMessage(threadId, text) { const sent = message(await apiRequest(`/conversations/${encodeURIComponent(threadId)}/messages`, { method: 'POST', data: { content: text } })); const cached = readSnapshot<ChatThread>('thread', threadId); if (cached) await persistThread({ ...cached, updatedAt: sent.createdAt, messages: mergeMessages(cached.messages, [sent]) }, true); return sent },
  ...
  async listNotifications() { const values = (await apiRequest<any[]>('/notifications')).map((value): Notification => ({ id: String(value.id), type: (['like', 'comment', 'follow'].includes(value.type) ? value.type : 'system') as Notification['type'], title: String(value.title || '系统通知'), subtitle: String(value.body || ''), unread: value.readAt ? 0 : 1, createdAt: String(value.createdAt || '') })); await writeSnapshot('notifications', values); return values },
