...
type SnapshotKind = 'favorites' | 'my-listings' | 'profile'
function snapshotKey(kind: SnapshotKind) { return `api-snapshot:${kind}:${sessionStore.peek()?.user.id || 'anonymous'}` }
function readSnapshot<T>(kind: SnapshotKind) { return storageAdapter.peek<T | undefined>(snapshotKey(kind), undefined) }
async function writeSnapshot<T>(kind: SnapshotKind, value: T) { await storageAdapter.set(snapshotKey(kind), value) }
