...
export const apiRepository: DemoRepository = {
  ...
  async getThread(id) { const current = await sessionStore.get(); const encodedId = encodeURIComponent(id); const [messages, allResponse] = await Promise.all([apiRequest<{ items: any[]; olderCursor?: string | null; blocked?: boolean }>(`/conversations/${encodedId}/messages` + apiQuery({ limit: 30 })), apiRequest<{ items: any[] } | any[]>('/conversations' + apiQuery({ limit: 50 }))]); const all = Array.isArray(allResponse) ? allResponse : allResponse.items; const found = all.find((item) => item.id === id); if (!found) throw new Error('会话不存在'); const fresh = thread({ ...found, messages: messages.items, olderCursor: messages.olderCursor, blocked: messages.blocked ?? found.blocked }, current?.user.id); const cached = readSnapshot<ChatThread>('thread', id); const value = cached ? { ...fresh, olderCursor: cached.olderCursor ?? fresh.olderCursor, messages: mergeMessages(cached.messages, fresh.messages) } : fresh; const latest = messages.items?.at(-1); if (latest?.id) await apiRequest(`/conversations/${encodedId}/read`, { method: 'POST', data: { messageId: String(latest.id) } }).catch(() => undefined); value.unread = 0; await persistThread(value); return value },
