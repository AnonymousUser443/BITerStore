...
import { beginNavigationFeedback, getNavigationChromeStyle, markNavigationReady, navigationAdapter } from '@/platform'
...
export function BrandHeader({ title, back = false, backTo, action }: { title?: string; back?: boolean; backTo?: string; action?: ReactNode }) {
  ...
  return <View style={getNavigationChromeStyle()} className={`topbar brand-header ${title ? 'page-topbar page-header' : ''} ${back ? 'back-header' : 'root-header'}`}>
    ...
    <View className='top-actions header-actions'>{action || <><Button id='e2e-header-notifications' className='icon-button header-icon' onClick={() => navigationAdapter.switchTab('/pages/messages/index')}><Glyph name='bell' /><Text className='notification-dot' /></Button>{!back && !title && <Button id='e2e-header-profile' className='avatar-action' onClick={() => navigationAdapter.switchTab('/pages/profile/index')}><Avatar user={current} size={38} /></Button>}</>}</View>
  </View>
}
...
function AppNavigation({ active }: { active?: string }) {
  return <View className='bottom-nav app-navigation'>{appTabs.map(([route, label, icon]) => <Button key={route} id={`e2e-nav-${route}`} className={`nav-item ${active === route ? 'active' : ''} ${route === 'publish' ? 'publish' : ''}`} onClick={() => navigationAdapter.switchTab(`/pages/${route}/index`)}><View className={`nav-icon ${icon}`} /><Text>{label}</Text></Button>)}</View>
}
...
export function AppShell({ children, title, back, backTo, active, className = '', noNav = false, scrollIntoView, overlay }: PropsWithChildren<{ title?: string; back?: boolean; backTo?: string; active?: string; className?: string; noNav?: boolean; scrollIntoView?: string; overlay?: ReactNode }>) {
  ...
  const contentClassName = `content-scroll page-scroll route-view ${hideNavigation ? 'no-nav' : ''}`
  ...
  return <View className={`phone-shell app-shell ${className}`}><Image className='paper-texture' src={bundledAsset('paper-bg')} mode='aspectFill' /><BrandHeader title={title} back={shouldGoBack} backTo={destination} />{content}{overlay}{!hideNavigation && <AppNavigation active={active} />}</View>
}
...
export function StatusTag({ status }: { status: ListingStatus }) { const labels: Record<ListingStatus, string> = { available: '可交易', sold: '已售', offline: '已下架', draft: '草稿', reviewing: '待审核', changes_requested: '待修改' }; return <Text className={`status-tag ${status}`}>{labels[status]}</Text> }
...
export function FilterDrawer({ open, children, onClose }: PropsWithChildren<{ open: boolean; onClose: () => void }>) { if (!open) return null; return <View className='sheet-layer'><Button className='sheet-scrim' aria-label='关闭筛选' onClick={onClose} /><View className='filter-sheet'>{children}</View></View> }
